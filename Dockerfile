...
RUN if [[ $IS_PR -eq 0 ]]; then \
    ...
    ./slcli config create-bsid --app "frontend" --build "$BUILD_NAME" --branch "master" ; \
else \ 
    echo "Pull request"; \
    ./slcli config create-pr-bsid --app "frontend" --branch REMOVE-THIS --build "${BUILD_NAME}" --target-branch "${TARGET_BRANCH}" \
